...
// Get all SPOCs with university short names for client-side search
$allSpocs = Spoc::getAllWithUniversity($conn);
            ...
            <div class="col-md-12 grid-margin">
                <div class="row">
                    <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                        <h3 class="font-weight-bold">Manage SPOCs</h3>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card shadow">
                    <div class="card-body">
                        <p class="card-title mb-0" style="font-size:larger">SPOCs</p><br>
                        ...
                        <div class="table-responsive">
                            <div class="input-group mb-3">
                                <input class="form-control" id="searchInput" type="text" placeholder="🔍 Search SPOC by Name, Email or University..." style="height: 38px; border-radius: 5px;">
                            ...
                            <form id="spocForm" method="post" action="/admin/delete_spocs">
...
<script>
    ...
    function bulkResetPassword() {
        ...
        var newPassword = prompt("Enter new password for selected SPOCs:");
        if (newPassword) {
            var confirmPassword = prompt("Confirm new password:");
            if (newPassword === confirmPassword) {
                $.ajax({
                    type: 'POST',
                    url: '/admin/bulk_reset_spoc_password',
...
const allRecords = <?= json_encode($allSpocs) ?>;
let currentPage = 1;
...
function generateTableRow(spoc, index) {
    const daysAgo = spoc.last_login ? calculateDaysAgo(spoc.last_login) : 'N/A';
    return `
        <tr>
            <td><input type="checkbox" name="selected[]" value="${spoc.id}"></td>
            <td>${index + 1}</td>
            <td>${spoc.name}</td>
            <td>${spoc.email}</td>
            <td>${spoc.university_short_name}</td>
            <td>${daysAgo}</td>
            <td>
                <a href="view_university/${spoc.university_id}" class="btn btn-outline-primary btn-sm">
...
function filterAndDisplayRecords() {
    ...
    // Filter records
    filteredRecords = allRecords.filter(spoc => {
        return spoc.name.toLowerCase().includes(searchTerm) ||
               spoc.email.toLowerCase().includes(searchTerm) ||
               spoc.university_short_name.toLowerCase().includes(searchTerm);
    });
...
function displayCurrentPage() {
    ...
    const tableBody = document.getElementById('spocTableBody');
    tableBody.innerHTML = recordsToShow.map((spoc, idx) => 
        generateTableRow(spoc, startIndex + idx)
    ).join('');
    ...
    table.addEventListener('click', function(e) {
        ...
        if (row && !row.closest('thead')) {
            ...
            if (!e.target.closest('a') && !e.target.closest('button') && !e.target.closest('input')) {
                ...
                const viewButton = row.querySelector('a[href*="view_university"]');
                if (viewButton) {
