...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sqlVerify = "SELECT password FROM spocs WHERE email = ?";
    $stmtVerify = $conn->prepare($sqlVerify);
    $stmtVerify->bind_param("s", $email);
    $stmtVerify->execute();
    ...
    if ($stmtVerify->fetch() && password_verify($currentPassword, $storedPassword)) {
        ...
        $sqlUpdate = "UPDATE spocs SET password = ? WHERE email = ?";
        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bind_param("ss", $hashedNewPassword, $email);

        if ($stmtUpdate->execute()) {
            // Password updated successfully
            echo "<script>
                    alert('Password updated successfully.');
                    window.location.href = 'change_password'; // Redirect to another page after success
                </script>";
            exit();
        } else {
